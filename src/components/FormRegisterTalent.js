...
class FormRegisterTalent extends Component {
  render () {
    const { msg, fullName, email, phoneNumber, password, confirmPassword, changeFullName, changeEmail, changePhoneNumber, changePassword, changeConfirmPassword, onRegister } = this.props
    return (
      <React.Fragment>
        <div className="flex flex-col mt-24 mr-12">
          ...
          {msg !== ''
            ? msg !== 'success register talent!' && <div className='bg-red-300 text-red-600 font-bold w-full mt-5 px-2 py-5'>{msg}</div>
            : <></>
          }
...
export default FormRegisterTalent;
